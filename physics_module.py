...
def load(settings, **kwargs):
    ...
    settings["ADDONS_DIR"] = env(
        "ADDONS_DIR", os.path.join(settings["BASE_DIR"], "addons")
    )
    settings["ADDONS_DEV_DIR"] = env(
        "ADDONS_DEV_DIR", os.path.join(settings["BASE_DIR"], "addons-dev")
    )
...
def load_addon_settings(name, path, settings, **kwargs):
    ...
    settings_json_path = kwargs.get(
        "settings_json_path", os.path.join(path, "config.json")
    )
